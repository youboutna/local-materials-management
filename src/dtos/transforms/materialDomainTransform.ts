...
export class MaterialDomainTransformer implements EntityToDTOMapper<Material, MaterialDTO> {
  ...
  // Utility methods for material operations
  static calculateMaterialValue(material: MaterialDTO): number {
    return material.availableQuantity * material.pricePerUnit;
  }

  static getMaterialStatus(material: MaterialDTO): 'in_stock' | 'low_stock' | 'out_of_stock' | 'discontinued' {
    if (material.availableQuantity <= 0) return 'out_of_stock';
    if (material.availableQuantity <= 10) return 'low_stock'; // Low stock threshold
    return 'in_stock';
  }

  static calculateReorderPoint(material: MaterialDTO): number {
    return 10; // Fixed reorder point for simplicity
  }

  static isMaterialPreferred(material: MaterialDTO): boolean {
    return material.pricePerUnit < 100 && material.availableQuantity > 0; // Good price and available
  }
}
