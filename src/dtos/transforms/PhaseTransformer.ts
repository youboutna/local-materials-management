...
export class PhaseTransformer {
  ...
  static stepToDTO(step: PhaseStep): PhaseStepDTO {
    return {
      ...
      actualDurationDays: step.actualDurationDays,
      startDate: step.startDate ? new Date(step.startDate).toISOString().split('T')[0] : undefined,
      endDate: step.endDate ? new Date(step.endDate).toISOString().split('T')[0] : undefined,
  ...
  static dtoToTask(dto: PhaseTaskDTO): PhaseTask {
    return {
      ...
      estimated_duration_days: dto.estimatedDurationDays,
      actual_duration_days: dto.actualDurationDays,
