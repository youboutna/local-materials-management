...
export function useProjectCheckpoints(projectId: string | undefined): ProjectCheckpointsResult {
  ...
  const computed = useMemo(() => {
    ...
    for (const phase of phases) {
      ...
      const phasePaid = phasePayments.reduce((sum: number, p: any) => sum + (p.amount || 0), 0);
      ...
      const hasPV = phaseDocuments.some((d: any) => d.document_type?.includes('pv'));
      
      const phaseVerification: ProjectVerificationSummary = {
        allVerified: hasCompletedInspection && hasDocuments,
        verificationScore: (hasCompletedInspection ? 25 : 0) + (hasDocuments ? 25 : 0) + (hasPV ? 50 : 0),
        inspectionVerified: hasCompletedInspection,
        documentVerified: hasDocuments,
        approvalVerified: true,
        pvVerified: hasPV,
        canRequestPayment: hasCompletedInspection && (phase.progress || 0) >= 25,
      };
      ...
      const phaseDecompte: PhaseDecompteSummary = {
        ...
        netPayable,
        canRequestPayment: netPayable > 0 && phaseVerification.inspectionVerified,
      };
      ...
      if (phaseBudget > 0) {
        weightedProgress += (phase.progress || 0) * phaseBudget;
    ...
    const projectVerificationResult: ProjectVerificationSummary = {
      ...
      canRequestPayment: allPhasesVerified && pendingCheckpoints === 0,
      verificationScore: completedCheckpoints > 0 
        ? Math.round((completedCheckpoints / (completedCheckpoints + pendingCheckpoints)) * 100)
        : 0,
      inspectionVerified: inspections.some((i: any) => i.status === 'completed'),
      documentVerified: documents.length > 0,
      approvalVerified: true,
      pvVerified: documents.some((d: any) => d.document_type?.includes('pv')),
    };
