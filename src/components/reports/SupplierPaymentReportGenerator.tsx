...
import { DollarSign, Download, Mail, Loader2, CreditCard } from 'lucide-react';
...
export function SupplierPaymentReportGenerator({ 
  ...
  const generatePDF = async () => {
    setLoading(true);
    try {
      const reportHTML = generatePaymentReportContent();
      ...
      const canvas = await html2canvas(tempDiv.querySelector('#payment-report-content') as HTMLElement, {
        scale: 2,
        useCORS: true,
        allowTaint: true,
      });
