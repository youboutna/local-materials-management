...
export class CheckpointActionContextService {
  ...
  async getActionableMilestones(projectId: string, phaseId?: string): Promise<MilestoneSummaryDTO[]> {
    ...
    if (phaseId) {
      ...
      milestones = await MilestoneService.getProjectMilestones(projectId);
    } else {
      milestones = await MilestoneService.getProjectMilestones(projectId);
    }
  ...
  private mapPhaseToSummary(phase: any): PhaseSummary {
    return {
      id: phase.id,
      name: phase.title || phase.phase_name || phase.name,
      status: phase.status,
      progress: phase.progress,
      startDate: phase.startDate || phase.start_date,
      endDate: phase.endDate || phase.end_date,
      estimatedCost: phase.budget || phase.estimated_cost,
      actualCost: phase.actualCost || phase.actual_cost,
      steps: [] // For now, empty steps since PhaseData doesn't have steps
    };
  }

  private determineInspectionType(milestone: any): 'technical' | 'quality' | 'safety' | 'regulatory' {
