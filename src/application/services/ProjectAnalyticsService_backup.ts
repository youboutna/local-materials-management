...
// Extended interfaces for analytics types
...
interface ExtendedProgressAnalytics {
...
interface ExtendedRiskAnalytics {
  overallRiskScore?: number;
  riskFactors: string[];
  riskLevel: 'low' | 'medium' | 'high';
}

interface ExtendedQualityMetrics {
  qualityScore?: number;
...
export class ProjectAnalyticsService {
  ...
  /**
   * Convert InspectionDTO to ProjectInspection format
   */
  private convertToProjectInspection(inspection: InspectionDTO): ProjectInspection {
    return {
      id: inspection.id,
      ...
      title: `Inspection ${inspection.id}`, // InspectionDTO doesn't have title
      description: inspection.comments || '', // Use comments as description
      date: inspection.date,
  ...
  async getProjectAnalytics(projectId: string): Promise<ProjectAnalyticsDTO> {
    try {
      ...
      const progressAnalytics = ProjectCalculationService.calculateProgressAnalytics(projectDetailDTO) as unknown as ExtendedProgressAnalytics;
      ...
      const qualityMetrics = ProjectCalculationService.calculateQualityMetrics(projectDetailDTO) as unknown as ExtendedQualityMetrics;
      const riskAnalytics = ProjectCalculationService.calculateRiskAnalytics(projectDetailDTO) as unknown as ExtendedRiskAnalytics;
      ...
      return {
        ...
        milestone_completion: progressAnalytics.milestonesProgress || 0,
        risk_score: riskAnalytics.overallRiskScore || 0,
