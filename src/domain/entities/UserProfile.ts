...
export class UserProfile {
  ...
  isActive(): boolean { return this._status === ProfileStatus.ACTIVE; }
  isPendingVerification(): boolean { return this._status === ProfileStatus.PENDING_VERIFICATION; }
  isSuspended(): boolean { return this._status === ProfileStatus.SUSPENDED; }
  canAccessSystem(): boolean { return this._status === ProfileStatus.ACTIVE || this._status === ProfileStatus.PENDING_VERIFICATION; }
  hasAdminPrivileges(): boolean { return this._isAdmin && this.isActive(); }

  updateFullName(newFullName: string): void {
    const trimmed = newFullName.trim();
    if (trimmed.length === 0) throw new Error('Full name cannot be empty');
    this._fullName = trimmed;
  }
  updatePhone(newPhone?: string): void { this._phone = newPhone?.trim(); }
  updateNationalId(newNationalId?: string): void { this._nationalId = newNationalId?.trim(); }
  updateAvatarUrl(newAvatarUrl?: string): void { this._avatarUrl = newAvatarUrl?.trim(); }
  grantAdminPrivileges(): void { this._isAdmin = true; }
  revokeAdminPrivileges(): void { this._isAdmin = false; }
  activate(): void { if (this._status === ProfileStatus.ACTIVE) throw new Error('Already active'); this._status = ProfileStatus.ACTIVE; }
  suspend(): void { if (this._status === ProfileStatus.SUSPENDED) throw new Error('Already suspended'); this._status = ProfileStatus.SUSPENDED; }
  markAsPendingVerification(): void { this._status = ProfileStatus.PENDING_VERIFICATION; }
  updateLastLogin(): void { this._lastLoginAt = new Date(); }
  setLastLoginAt(lastLoginAt: Date): void { this._lastLoginAt = new Date(lastLoginAt); }
  updateDepartment(newDepartment?: string): void { this._department = newDepartment?.trim(); }
  updatePosition(newPosition?: string): void { this._position = newPosition?.trim(); }
  updateLocation(newLocation?: string): void { this._location = newLocation?.trim(); }
  ...
  validatePhoneNumber(): boolean {
    if (!this._phone) return true;
    const phoneRegex = /^\+?[\d\s\-\(\)]+$/;
    return phoneRegex.test(this._phone);
  }

  validateNationalId(): boolean {
    if (!this._nationalId) return true;
    return this._nationalId.length >= 6;
  }
  ...
  getDisplayName(): string { return this._fullName; }
  getShortInfo(): string {
