...
CREATE TABLE IF NOT EXISTS btp.quantity_takeoffs (
...
-- Workspaces Table
CREATE TABLE IF NOT EXISTS btp.workspaces (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name TEXT NOT NULL,
    description TEXT,
...
CREATE TABLE IF NOT EXISTS btp.project_comments (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    project_id UUID NOT NULL REFERENCES btp.projects(id) ON DELETE CASCADE,
    user_id UUID NOT NULL,
    comment TEXT NOT NULL,
    parent_comment_id UUID REFERENCES btp.project_comments(id),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Material Suppliers Junction Table
CREATE TABLE IF NOT EXISTS btp.material_suppliers (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
