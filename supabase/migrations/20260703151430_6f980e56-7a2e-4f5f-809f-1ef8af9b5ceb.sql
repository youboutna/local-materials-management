...
BEGIN
  FOREACH t IN ARRAY tables LOOP
    ...
    IF has_col THEN
      ...
      EXECUTE format(
        'CREATE TRIGGER set_updated_at BEFORE UPDATE ON btp.%I FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column()',
        t
      );
    END IF;
  END LOOP;
...
CREATE OR REPLACE FUNCTION public.get_escalation_targets(project_id_param uuid, escalation_level_param text)
