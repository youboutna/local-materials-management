...
ALTER TABLE materials ADD COLUMN last_restock TIMESTAMP WITH TIME ZONE;

-- Add material_status column (VARCHAR with enum-like constraint)
ALTER TABLE materials ADD COLUMN material_status VARCHAR(50) CHECK (material_status IN ('active', 'inactive', 'discontinued', 'pending'));

-- Add min_quantity column (INTEGER)
ALTER TABLE materials ADD COLUMN min_quantity INTEGER DEFAULT 0;
...
ALTER TABLE materials ADD CONSTRAINT chk_min_quantity_positive
