...
CREATE TABLE IF NOT EXISTS public.tender_sharing_secrets (
...
CREATE TABLE IF NOT EXISTS public.tender_sharing_access_logs (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  sharing_secret_id UUID NOT NULL REFERENCES public.tender_sharing_secrets(id) ON DELETE CASCADE,
...
ALTER TABLE public.tender_sharing_secrets ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.tender_sharing_access_logs ENABLE ROW LEVEL SECURITY;
...
ON public.tender_sharing_secrets
FOR ALL
USING (
  shared_by = auth.uid()
  OR EXISTS (
...
USING (
  ...
  OR supplier_id IN (
    SELECT id FROM public.suppliers WHERE email = auth.jwt()->>'email'
  )
);

-- Authenticated users can log access
CREATE POLICY "Authenticated users can create access logs"
ON public.tender_sharing_access_logs
FOR INSERT
WITH CHECK (true);
...
USING (
  EXISTS (
    SELECT 1 FROM public.tender_sharing_secrets tss
...
CREATE INDEX IF NOT EXISTS idx_tender_sharing_secrets_tender_id ON public.tender_sharing_secrets(tender_id);
CREATE INDEX IF NOT EXISTS idx_tender_sharing_secrets_secret_code ON public.tender_sharing_secrets(secret_code);
CREATE INDEX IF NOT EXISTS idx_tender_sharing_secrets_expires_at ON public.tender_sharing_secrets(expires_at);
CREATE INDEX IF NOT EXISTS idx_tender_sharing_access_logs_secret_id ON public.tender_sharing_access_logs(sharing_secret_id);

-- Add updated_at trigger
DROP TRIGGER IF EXISTS update_tender_sharing_secrets_updated_at ON public.tender_sharing_secrets;
CREATE TRIGGER update_tender_sharing_secrets_updated_at
BEFORE UPDATE ON public.tender_sharing_secrets
FOR EACH ROW
EXECUTE FUNCTION public.update_updated_at_column();

-- Function to generate unique secret code
CREATE OR REPLACE FUNCTION public.generate_tender_secret_code()
RETURNS TEXT AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.validate_tender_secret(secret_code_param TEXT)
RETURNS TABLE(
  is_valid BOOLEAN,
  tender_id UUID,
...
BEGIN
  ...
  FROM public.tender_sharing_secrets
  ...
  UPDATE public.tender_sharing_secrets
