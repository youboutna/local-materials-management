...
DROP POLICY IF EXISTS "Allow all access to payment_blocks" ON public.payment_blocks;
DROP POLICY IF EXISTS "Allow all operations on tender_estimate_items" ON public.tender_estimate_items;
DROP POLICY IF EXISTS "Allow all operations on tender_estimates" ON public.tender_estimates;

-- 3) tender_workflow_status: restrict SELECT
DROP POLICY IF EXISTS "Users can view tender workflow status" ON public.tender_workflow_status;
CREATE POLICY "Admins and participants can view tender workflow status"
  ON public.tender_workflow_status FOR SELECT
  TO authenticated
  USING (
    public.is_current_user_admin()
    OR EXISTS (
      SELECT 1 FROM public.tender_estimates te
...
DROP POLICY IF EXISTS "Authenticated users can manage workflow status" ON public.workflow_status;
...
ALTER FUNCTION public.admin_update_user_email(uuid, text) SET search_path = public;
ALTER FUNCTION public.assign_user_role(uuid, text) SET search_path = public;
ALTER FUNCTION public.generate_supplier_reset_token(text) SET search_path = public;
ALTER FUNCTION public.get_user_role(uuid) SET search_path = public;
ALTER FUNCTION public.get_user_roles(uuid) SET search_path = public;
ALTER FUNCTION public.handle_new_user() SET search_path = public;
ALTER FUNCTION public.handle_oauth_user_profile() SET search_path = public;
...
REVOKE EXECUTE ON FUNCTION public.generate_supplier_reset_token(text) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.create_progress_invoice(uuid, uuid, numeric, numeric, text, jsonb, jsonb) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.create_station_from_authorization(uuid, uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.create_supplier_payment_request(uuid, numeric, text, text, uuid, text[], text) FROM PUBLIC, anon;
