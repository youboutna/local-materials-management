...
CREATE TABLE btp.insurance_certificates (
...
CREATE TABLE btp.bank_guarantees (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID NOT NULL,
  contractor_id UUID NOT NULL,
...
CREATE TABLE btp.payment_blocks (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID NOT NULL,
  contractor_id UUID NOT NULL,
...
-- Create policies
CREATE POLICY "Allow all access to insurance_certificates" ON btp.insurance_certificates FOR ALL USING (true);
CREATE POLICY "Allow all access to bank_guarantees" ON btp.bank_guarantees FOR ALL USING (true);
CREATE POLICY "Allow all access to payment_blocks" ON btp.payment_blocks FOR ALL USING (true);