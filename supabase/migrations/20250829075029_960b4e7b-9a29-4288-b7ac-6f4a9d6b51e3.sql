-- First, fix the tender_estimates table to use user_id instead of supplier_id
...
-- Create tender document submissions table using user_id instead of supplier_id
CREATE TABLE IF NOT EXISTS public.tender_document_submissions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tender_id UUID NOT NULL,
...
ALTER TABLE public.tender_document_submissions ENABLE ROW LEVEL SECURITY;

-- Users can view their own submissions
CREATE POLICY "Users can view their own submissions"
ON public.tender_document_submissions
FOR SELECT
USING (submitted_by = auth.uid());
...
-- Admins can update all submissions (for reviews)
...
CREATE OR REPLACE TRIGGER update_tender_document_submissions_updated_at
  BEFORE UPDATE ON public.tender_document_submissions
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_column();

CREATE OR REPLACE TRIGGER update_tender_estimates_updated_at
  BEFORE UPDATE ON public.tender_estimates
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_column();

CREATE OR REPLACE TRIGGER update_tender_estimate_items_updated_at
  BEFORE UPDATE ON public.tender_estimate_items
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_column();