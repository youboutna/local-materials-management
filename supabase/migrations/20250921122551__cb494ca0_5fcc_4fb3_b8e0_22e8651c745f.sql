...
-- Add missing columns to project_risks table for enhanced risk management (fix existing text columns first)
...
-- Create enhanced project milestones table for tracking key deliverables
CREATE TABLE IF NOT EXISTS enhanced_project_milestones (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  title TEXT NOT NULL,
  description TEXT,
...
-- Enable RLS on enhanced_project_milestones
ALTER TABLE enhanced_project_milestones ENABLE ROW LEVEL SECURITY;

-- Create policy for enhanced_project_milestones
CREATE POLICY "Users can manage milestones for accessible projects" ON enhanced_project_milestones
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM projects p 
      WHERE p.id = enhanced_project_milestones.project_id
    )
  );
...
CREATE INDEX IF NOT EXISTS idx_enhanced_project_milestones_project_id ON enhanced_project_milestones(project_id);
CREATE INDEX IF NOT EXISTS idx_enhanced_project_milestones_phase_id ON enhanced_project_milestones(phase_id);
...
DROP TRIGGER IF EXISTS update_enhanced_project_milestones_updated_at ON enhanced_project_milestones;
CREATE TRIGGER update_enhanced_project_milestones_updated_at
  BEFORE UPDATE ON enhanced_project_milestones
  FOR EACH ROW
