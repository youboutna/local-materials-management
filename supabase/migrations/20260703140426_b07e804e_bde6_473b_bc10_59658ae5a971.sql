...
-- ============================================================
CREATE TABLE IF NOT EXISTS btp.tender_estimates (LIKE btp.tender_estimates INCLUDING ALL);
CREATE TABLE IF NOT EXISTS btp.tender_estimate_items (LIKE btp.tender_estimate_items INCLUDING ALL);
CREATE TABLE IF NOT EXISTS btp.tender_document_submissions (LIKE btp.tender_document_submissions INCLUDING ALL);
CREATE TABLE IF NOT EXISTS btp.tender_submission_documents (LIKE btp.tender_submission_documents INCLUDING ALL);
CREATE TABLE IF NOT EXISTS btp.tender_suppliers (LIKE btp.tender_suppliers INCLUDING ALL);
CREATE TABLE IF NOT EXISTS btp.tender_workflow_status (LIKE btp.tender_workflow_status INCLUDING ALL);
CREATE TABLE IF NOT EXISTS btp.tender_sharing_access_logs (LIKE btp.tender_sharing_access_logs INCLUDING ALL);
CREATE TABLE IF NOT EXISTS btp.submission_access_logs (LIKE btp.submission_access_logs INCLUDING ALL);
...
-- ============================================================
INSERT INTO btp.tender_estimates              SELECT * FROM btp.tender_estimates              ON CONFLICT (id) DO NOTHING;
INSERT INTO btp.tender_estimate_items         SELECT * FROM btp.tender_estimate_items         ON CONFLICT (id) DO NOTHING;
INSERT INTO btp.tender_document_submissions   SELECT * FROM btp.tender_document_submissions   ON CONFLICT (id) DO NOTHING;
INSERT INTO btp.tender_submission_documents   SELECT * FROM btp.tender_submission_documents   ON CONFLICT (id) DO NOTHING;
INSERT INTO btp.tender_suppliers              SELECT * FROM btp.tender_suppliers              ON CONFLICT DO NOTHING;
INSERT INTO btp.tender_workflow_status        SELECT * FROM btp.tender_workflow_status        ON CONFLICT (id) DO NOTHING;
INSERT INTO btp.tender_sharing_access_logs    SELECT * FROM btp.tender_sharing_access_logs    ON CONFLICT (id) DO NOTHING;
INSERT INTO btp.submission_access_logs        SELECT * FROM btp.submission_access_logs        ON CONFLICT (id) DO NOTHING;

-- Backfill previously-created btp mirrors that may have drifted (idempotent)
INSERT INTO btp.tenders                       SELECT * FROM btp.tenders                       ON CONFLICT (id) DO NOTHING;
INSERT INTO btp.tender_submissions            SELECT * FROM btp.tender_submissions            ON CONFLICT (id) DO NOTHING;
INSERT INTO btp.tender_lots                   SELECT * FROM btp.tender_lots                   ON CONFLICT (id) DO NOTHING;
INSERT INTO btp.tender_lot_documents          SELECT * FROM btp.tender_lot_documents          ON CONFLICT (id) DO NOTHING;
INSERT INTO btp.tender_sharing_secrets        SELECT * FROM btp.tender_sharing_secrets        ON CONFLICT (id) DO NOTHING;
INSERT INTO btp.submission_activity_logs      SELECT * FROM btp.submission_activity_logs      ON CONFLICT (id) DO NOTHING;
INSERT INTO btp.document_validation_logs      SELECT * FROM btp.document_validation_logs      ON CONFLICT (id) DO NOTHING;
...
CREATE POLICY "auth_all" ON btp.tender_estimates FOR ALL TO authenticated
  USING (submitted_by = auth.uid() OR btp.is_current_user_admin())
  WITH CHECK (submitted_by = auth.uid() OR btp.is_current_user_admin());
...
CREATE POLICY "auth_all" ON btp.tender_estimate_items FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM btp.tender_estimates te WHERE te.id = estimate_id AND (te.submitted_by = auth.uid() OR btp.is_current_user_admin())))
  WITH CHECK (EXISTS (SELECT 1 FROM btp.tender_estimates te WHERE te.id = estimate_id AND (te.submitted_by = auth.uid() OR btp.is_current_user_admin())));
...
CREATE POLICY "own_read"  ON btp.tender_document_submissions FOR SELECT TO authenticated USING (submitted_by = auth.uid() OR btp.is_current_user_admin());
CREATE POLICY "own_write" ON btp.tender_document_submissions FOR INSERT TO authenticated WITH CHECK (submitted_by = auth.uid());
CREATE POLICY "admin_all" ON btp.tender_document_submissions FOR UPDATE TO authenticated USING (btp.is_current_user_admin());
...
CREATE POLICY "auth_read"  ON btp.tender_sharing_access_logs FOR SELECT TO authenticated USING (btp.is_current_user_admin());
...
BEGIN
  ...
  LOOP
    ...
    EXECUTE format('CREATE TRIGGER set_updated_at BEFORE UPDATE ON btp.%I FOR EACH ROW EXECUTE FUNCTION btp.update_updated_at_column()', t);
  END LOOP;
END $$;
