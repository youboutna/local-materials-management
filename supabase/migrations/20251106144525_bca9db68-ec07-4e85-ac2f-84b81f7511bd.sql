...
CREATE OR REPLACE FUNCTION public.get_submission_activity_logs(p_submission_id UUID)
RETURNS TABLE (
  id UUID,
  submission_id UUID,
  action TEXT,
...
BEGIN
  ...
  SELECT 
    ...
    sal.performed_by,
    sal.created_at
  FROM public.submission_activity_logs sal
...
CREATE OR REPLACE FUNCTION public.get_validation_logs(p_submission_id UUID)
RETURNS TABLE (
  id UUID,
  document_id UUID,
  submission_id UUID,
...
BEGIN
  ...
  SELECT 
    ...
    dvl.validated_at,
    dvl.created_at
  FROM public.document_validation_logs dvl
