...
AS $$
  ...
  FROM public.organizational_hierarchy oh
  JOIN public.employees e ON oh.employee_id = e.id
  JOIN public.organizations org ON oh.organization_id = org.id
  JOIN public.project_organizations po ON org.id = po.organization_id
...
CREATE OR REPLACE FUNCTION public.get_hierarchy_chain(employee_id_param uuid, direction text DEFAULT 'up')
RETURNS TABLE (
  hierarchy_id uuid,
  employee_id uuid,
  employee_name text,
...
CREATE OR REPLACE FUNCTION public.get_escalation_targets(
