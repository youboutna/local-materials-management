-- Add check_schedule_last_run column to projects table
ALTER TABLE public.projects 
ADD COLUMN check_schedule_last_run JSONB DEFAULT '{}'::jsonb;

-- Drop and recreate project_resources with simplified structure
DROP TABLE IF EXISTS public.project_resources CASCADE;

CREATE TABLE public.project_resources (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  project_id UUID NOT NULL REFERENCES public.projects(id) ON DELETE CASCADE,
  type TEXT NOT NULL CHECK (type IN ('human', 'material')),
  name TEXT NOT NULL,
  quantity INTEGER DEFAULT 1,
  unit TEXT,
  cost_per_unit DECIMAL(15,2),
  total_cost DECIMAL(15,2),
  allocation_date DATE,
  notes TEXT,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);

-- Create project_alerts table
CREATE TABLE public.project_alerts (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  project_id UUID NOT NULL REFERENCES public.projects(id) ON DELETE CASCADE,
  type TEXT NOT NULL,
  severity TEXT NOT NULL CHECK (severity IN ('low', 'medium', 'high', 'critical')),
  title TEXT NOT NULL,
  description TEXT,
  source TEXT,
  acknowledged BOOLEAN DEFAULT false,
  acknowledged_by UUID REFERENCES auth.users(id),
  acknowledged_at TIMESTAMP WITH TIME ZONE,
  resolved BOOLEAN DEFAULT false,
  resolved_by UUID REFERENCES auth.users(id),
  resolved_at TIMESTAMP WITH TIME ZONE,
  escalation_level INTEGER DEFAULT 1,
  assigned_actions TEXT[],
  action_proofs JSONB DEFAULT '[]'::jsonb,
  metadata JSONB DEFAULT '{}'::jsonb,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);

-- Create indexes for better performance
CREATE INDEX idx_project_resources_project_id ON public.project_resources(project_id);
CREATE INDEX idx_project_resources_type ON public.project_resources(type);
CREATE INDEX idx_project_alerts_project_id ON public.project_alerts(project_id);
CREATE INDEX idx_project_alerts_severity ON public.project_alerts(severity);
CREATE INDEX idx_project_alerts_acknowledged ON public.project_alerts(acknowledged);
CREATE INDEX idx_project_alerts_resolved ON public.project_alerts(resolved);

-- Enable Row Level Security
ALTER TABLE public.project_resources ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.project_alerts ENABLE ROW LEVEL SECURITY;

-- Create RLS policies for project_resources based on existing project access patterns
CREATE POLICY "Users can view project resources if they have project access" 
ON public.project_resources 
FOR SELECT 
USING (
  EXISTS (
    SELECT 1 FROM public.projects 
    WHERE projects.id = project_resources.project_id 
    AND projects.project_responsable_id = auth.uid()
  )
  OR EXISTS (
    SELECT 1 FROM public.project_organizations po
    JOIN public.organizational_hierarchy oh ON po.organization_id = oh.organization_id
    JOIN public.employees e ON oh.employee_id = e.id
    WHERE po.project_id = project_resources.project_id 
    AND e.user_id = auth.uid()
  )
);

CREATE POLICY "Users can create project resources if they have project access" 
ON public.project_resources 
FOR INSERT 
WITH CHECK (
  EXISTS (
    SELECT 1 FROM public.projects 
    WHERE projects.id = project_resources.project_id 
    AND projects.project_responsable_id = auth.uid()
  )
  OR EXISTS (
    SELECT 1 FROM public.project_organizations po
    JOIN public.organizational_hierarchy oh ON po.organization_id = oh.organization_id
    JOIN public.employees e ON oh.employee_id = e.id
    WHERE po.project_id = project_resources.project_id 
    AND e.user_id = auth.uid()
  )
);

CREATE POLICY "Users can update project resources if they have project access" 
ON public.project_resources 
FOR UPDATE 
USING (
  EXISTS (
    SELECT 1 FROM public.projects 
    WHERE projects.id = project_resources.project_id 
    AND projects.project_responsable_id = auth.uid()
  )
  OR EXISTS (
    SELECT 1 FROM public.project_organizations po
    JOIN public.organizational_hierarchy oh ON po.organization_id = oh.organization_id
    JOIN public.employees e ON oh.employee_id = e.id
    WHERE po.project_id = project_resources.project_id 
    AND e.user_id = auth.uid()
  )
);

CREATE POLICY "Users can delete project resources if they have project access" 
ON public.project_resources 
FOR DELETE 
USING (
  EXISTS (
    SELECT 1 FROM public.projects 
    WHERE projects.id = project_resources.project_id 
    AND projects.project_responsable_id = auth.uid()
  )
  OR EXISTS (
    SELECT 1 FROM public.project_organizations po
    JOIN public.organizational_hierarchy oh ON po.organization_id = oh.organization_id
    JOIN public.employees e ON oh.employee_id = e.id
    WHERE po.project_id = project_resources.project_id 
    AND e.user_id = auth.uid()
  )
);

-- Create RLS policies for project_alerts
CREATE POLICY "Users can view project alerts if they have project access" 
ON public.project_alerts 
FOR SELECT 
USING (
  EXISTS (
    SELECT 1 FROM public.projects 
    WHERE projects.id = project_alerts.project_id 
    AND projects.project_responsable_id = auth.uid()
  )
  OR EXISTS (
    SELECT 1 FROM public.project_organizations po
    JOIN public.organizational_hierarchy oh ON po.organization_id = oh.organization_id
    JOIN public.employees e ON oh.employee_id = e.id
    WHERE po.project_id = project_alerts.project_id 
    AND e.user_id = auth.uid()
  )
);

CREATE POLICY "Users can create project alerts if they have project access" 
ON public.project_alerts 
FOR INSERT 
WITH CHECK (
  EXISTS (
    SELECT 1 FROM public.projects 
    WHERE projects.id = project_alerts.project_id 
    AND projects.project_responsable_id = auth.uid()
  )
  OR EXISTS (
    SELECT 1 FROM public.project_organizations po
    JOIN public.organizational_hierarchy oh ON po.organization_id = oh.organization_id
    JOIN public.employees e ON oh.employee_id = e.id
    WHERE po.project_id = project_alerts.project_id 
    AND e.user_id = auth.uid()
  )
);

CREATE POLICY "Users can update project alerts if they have project access" 
ON public.project_alerts 
FOR UPDATE 
USING (
  EXISTS (
    SELECT 1 FROM public.projects 
    WHERE projects.id = project_alerts.project_id 
    AND projects.project_responsable_id = auth.uid()
  )
  OR EXISTS (
    SELECT 1 FROM public.project_organizations po
    JOIN public.organizational_hierarchy oh ON po.organization_id = oh.organization_id
    JOIN public.employees e ON oh.employee_id = e.id
    WHERE po.project_id = project_alerts.project_id 
    AND e.user_id = auth.uid()
  )
);

CREATE POLICY "Users can delete project alerts if they have project access" 
ON public.project_alerts 
FOR DELETE 
USING (
  EXISTS (
    SELECT 1 FROM public.projects 
    WHERE projects.id = project_alerts.project_id 
    AND projects.project_responsable_id = auth.uid()
  )
  OR EXISTS (
    SELECT 1 FROM public.project_organizations po
    JOIN public.organizational_hierarchy oh ON po.organization_id = oh.organization_id
    JOIN public.employees e ON oh.employee_id = e.id
    WHERE po.project_id = project_alerts.project_id 
    AND e.user_id = auth.uid()
  )
);

-- Create triggers for automatic timestamp updates
CREATE TRIGGER update_project_resources_updated_at
BEFORE UPDATE ON public.project_resources
FOR EACH ROW
EXECUTE FUNCTION public.update_updated_at_column();

CREATE TRIGGER update_project_alerts_updated_at
BEFORE UPDATE ON public.project_alerts
FOR EACH ROW
EXECUTE FUNCTION public.update_updated_at_column();