...
DROP POLICY IF EXISTS "Authenticated users can create task assignments" ON btp.task_assignments;
DROP POLICY IF EXISTS "Authenticated users can update task assignments" ON btp.task_assignments;
...
USING (
  ...
  assigned_to IN (SELECT id FROM btp.suppliers)
);
...
USING (
  ...
  assigned_to IN (SELECT id FROM btp.employees)
);
