...
-- Create policies
CREATE POLICY "Admins can manage supplier notifications" ON public.supplier_notifications
  FOR ALL USING (
    EXISTS (
...
ALTER TABLE public.suppliers ADD COLUMN IF NOT EXISTS default_password_reset_required BOOLEAN DEFAULT true;

-- Update task_assignments to include completion_token
ALTER TABLE public.task_assignments ADD COLUMN IF NOT EXISTS completion_token TEXT;
ALTER TABLE public.task_assignments ADD COLUMN IF NOT EXISTS completion_url TEXT;
