...
CREATE INDEX idx_suppliers_nif ON btp.suppliers(nif);

-- Add an index for contact_person search performance
CREATE INDEX idx_suppliers_contact_person ON btp.suppliers(contact_person);

-- Add an index for name search performance
CREATE INDEX idx_suppliers_name ON btp.suppliers(name);