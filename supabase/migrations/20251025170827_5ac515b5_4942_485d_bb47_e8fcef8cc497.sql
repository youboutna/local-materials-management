...
CREATE INDEX IF NOT EXISTS idx_tender_submissions_secret_code ON btp.tender_submissions(secret_code) WHERE secret_code IS NOT NULL;

-- Create submission access logs table
CREATE TABLE IF NOT EXISTS btp.submission_access_logs (
...
CREATE INDEX IF NOT EXISTS idx_submission_access_logs_submission ON btp.submission_access_logs(submission_id);
CREATE INDEX IF NOT EXISTS idx_submission_access_logs_accessed_at ON btp.submission_access_logs(accessed_at DESC);

-- Enable RLS on submission_access_logs
ALTER TABLE btp.submission_access_logs ENABLE ROW LEVEL SECURITY;

-- RLS policies for submission_access_logs
CREATE POLICY "Users can view access logs for their submissions"
ON btp.submission_access_logs FOR SELECT
USING (
  EXISTS (
...
ON btp.submission_access_logs FOR INSERT
WITH CHECK (true);
...
BEGIN
  LOOP
    ...
    SELECT EXISTS (
      SELECT 1 FROM btp.tender_submissions
...
BEGIN
  ...
  FROM btp.tender_submissions ts
...
COMMENT ON TABLE btp.submission_access_logs IS 'Logs des accès aux dossiers de soumission pour audit et traçabilité';
